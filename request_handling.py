...
TELEGRAM_FILE_WEIGHT_LIMIT = 20 * 1024 * 1024
...
async def print_max_weight_error(update, weight):
    await update.message.reply_text(
        f"Файл имеет слишком большой объём. Существует ограничение на размер файла в {weight}. Отправьте файл меньшего размера.")
...
async def detection_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if update.message.audio is not None:
            if update.message.audio.file_size <= TELEGRAM_FILE_WEIGHT_LIMIT:
                ...
                await print_max_weight_error(update, "20 Мбайт")
                return 1

        # telegram video
        elif update.message.video is not None:
            if update.message.video.file_size <= TELEGRAM_FILE_WEIGHT_LIMIT:
        ...
        # telegram document
            ...
            if update.message.document.file_size <= TELEGRAM_FILE_WEIGHT_LIMIT:
                await update.message.reply_text("Скачивание...")
        ...
        # В случае, если пользователь отправил ссылку
            ...
            if update.message.text[:32] == "https://www.youtube.com/watch?v=":
                ...
                if video_dur <= FILE_DURATION_LIMIT:
                    try:
                        # Скачиваем с помощью командной утилиты yt-dlp
            ...
            # Загрузка файла с Google.Drive
                ...
                if int(information["size"]) <= WEIGHT_FILE_LIMIT:
                    ...
                    await print_max_weight_error(update, "5 Гбайт")
                    return 1
